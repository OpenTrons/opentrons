...
from typing import ContextManager, Any, List
...
@pytest.mark.parametrize(
    argnames=["nozzle_map", "expected_well_names"],
    argvalues=[
        (
            NozzleMap.build(
...
def test_grouping_wells_for_column_96_plate(
    nozzle_map: NozzleMap, expected_well_names: List[str], decoy: Decoy
) -> None:
    """It should group two columns into A1 and A2."""
    mock_wells = [decoy.mock(cls=Well) for _ in range(16)]
    mock_parent_labware = decoy.mock(cls=Labware)
    decoy.when(mock_parent_labware.parameters).then_return({"format": "96Standard"})  # type: ignore[typeddict-item]
    ...
    decoy.when(mock_parent_labware.columns()).then_return(labware_wells_by_column)

    for mock_well, well_name in zip(
        mock_wells, WELLS_BY_COLUMN_96[0] + WELLS_BY_COLUMN_96[1]
    ):
        decoy.when(mock_well.well_name).then_return(well_name)
        decoy.when(mock_well.parent).then_return(mock_parent_labware)

    wells = group_wells_for_multi_channel_transfer(mock_wells, nozzle_map)
    assert len(wells) == 2
    assert wells[0].well_name == expected_well_names[0]
    assert wells[1].well_name == expected_well_names[1]
...
def test_grouping_wells_for_column_384_plate(
    ...
    mock_parent_labware = decoy.mock(cls=Labware)
    decoy.when(mock_parent_labware.parameters).then_return({"format": "384Standard"})  # type: ignore[typeddict-item]
    labware_wells_by_column = []
    ...
    assert wells[0].well_name == expected_well_names[0]
    assert wells[1].well_name == expected_well_names[1]
    assert wells[2].well_name == expected_well_names[2]
    assert wells[3].well_name == expected_well_names[3]
...
def test_grouping_wells_for_full_96_plate(decoy: Decoy) -> None:
...
def test_grouping_wells_for_full_384_plate(decoy: Decoy) -> None:
